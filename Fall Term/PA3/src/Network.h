...
private:
    ...
    Client readClient(ifstream& file);
    void addRoutingInformationToClient(Client& client, ifstream& file);
    void printCommandDisplay(const string& command);

    Client* getClient(const string &clientID, vector<Client> &clients);
};
